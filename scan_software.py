...
def extract_assets(html, base_url):
    ...
    # PHP files (heuristic via <a> tags)
...
def crawl_website(url, max_depth, headers, domain, visited, results):
    ...
    results[normalized_url] = {
        ...
        "secrets": secrets,
        "links": [],
        "static_warnings": [],  # To be populated later by static analysis
    }
...
# ---------------------------
# Detect File Root Class via Extension
# ---------------------------
...
def get_page_status(data):
    if data["secrets"]:
        ...
        return "Neutral"   # Some assets; no overt secrets
    else:
        return "Good"      # Minimal content found
...
def heuristic_analysis(results, misconfigs):
    ...
    # Increase risk score for pages with secrets.
    for page, data in results.items():
        if data["secrets"]:
            score += 2
            details.append(f"{page}: Secrets found (+2)")
        # Static warnings from code analysis (if any)
    ...
    # Add penalties for misconfigurations from header analysis.
...
# Store results in SQLite and generate an HTML report.
# ---------------------------
...
def main():
    parser = argparse.ArgumentParser(
        description="Deep Domain Scanner & Security Audit Tool (with persistent storage, static analysis, misconfiguration lookup, and heuristic risk analysis)"
    )
    ...
    # Data structures to hold our scan results.
    visited = set()
    results = {}

    # Run the deep crawler with animated spinner.
    ...
    # Advanced Misconfiguration Lookup from HTTP Headers.
    ...
    console.print("\n[bold green]Security audit and scanning complete![/bold green]")
